...
function startEditor() {
    ...
    // 设置z-index
    editor.customConfig.zIndex = 1003;
...
function addDesi(){
    $.ajax({
        url:globalInterface.getAllDesi,
        async:false,
        type:"GET",
        success:(r) => {
            const obj = r.result;
...
function fillData() {
    ...
    $.get(globalInterface.getWorkAllByWorkId,{worksId:globalData.currentProfiloId},(r) => {
        const w = r.result;
        $(".profiloNameCh").val(w.worksName);
        $(".profiloNameEn").val(w.worksEnName);
        $(".preImg").attr("src", w.coverPath);
        addFirstCate();
        $(".firstCate").val(w.parentCateId);
        addSecondCate(w.cateId);
        addDesi();
        $(".desi").val(w.desiId);
        $(".updateTime").val(w.updateTime);
        $(".createTime").val(w.createTime);
        globalData.editor.txt.html(w.worksContent);
    })
}
...
function doPreLook(){
    ...
    $(".popupInner2").empty();
    // 设置弹窗标题
    $(".popupTitle2").html("作品预览");
    // 加载预览内容
    let inner = globalData.editor.txt.html();
    $(".popupInner2").html(inner);
}
...
function doSubmit(){
    ...
    const img = $(".upLoadPreImg").prop("files")[0];
    if (img!=null||typeof img != "undefined"){
        params.append("coverImg",img);
    }
    ...
    $.ajax({
        ...
        success:(result) => {
            ...
            // 重新加载页面
            $(".rightInnerArea").load(globalPath.updateProfilo);
            // 立即关闭弹窗
